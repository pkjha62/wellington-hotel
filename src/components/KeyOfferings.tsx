    ...
    <div id={id} className={`flex flex-col ${reverse ? "lg:flex-row-reverse" : "lg:flex-row"} min-h-[350px] sm:min-h-[500px] lg:min-h-[600px]`}>
      ...
      <div className="w-full lg:w-1/2 flex items-center">
        ...
        >
          ...
          <a href={`#${id}`} className="inline-flex items-center gap-2 mt-6 sm:mt-8 font-sans text-xs tracking-[0.2em] uppercase text-gold hover:text-gold-dark transition-colors duration-300 group/link">
            More information
  ...
  return (
    <section className="bg-offwhite">
      {offerings.map((offering, index) => (
        <OfferingBlock key={offering.id} {...offering} reverse={index % 2 === 1} />
      ))}
    </section>
  );
}
